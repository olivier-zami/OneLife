...
extern OneLife::game::LoginScreen* loginScreen;;
...
void LivingLifePage::readMessage(OneLife::data::type::message::MapChunk mapChunk, const char* message)
{
	...
	/***************************************/
	if(!this->localMap->isGlobalOffsetSet()) this->localMap->setGlobalOffset({x,y});
	this->localMap->applyOffset(&x, &y);
...
LivingLifePage::~LivingLifePage()
{
	OneLife::debug::Console::showFunction("LivingLifePage::~LivingLifePage() : Total received = %d bytes (+%d in headers), total sent = %d bytes (+%d in headers)\n",
...
void LivingLifePage::step()
{
    ...
    while( message != NULL )
	{
        ...
        if( mapPullMode && type != MESSAGE_TYPE::MAP_CHUNK )
		{
        ...
        else if( type == MESSAGE_TYPE::VALLEY_SPACING ) {
            sscanf( message, "VS\n%d %d",
                    &valleySpacing, &valleyOffset );
            }
        else if( type == MESSAGE_TYPE::FLIGHT_DEST ) {
            int posX, posY, playerID;
        ...
        else if( type == MESSAGE_TYPE::VOG_UPDATE ) {
            int posX, posY;
        ...
        else if( type == MESSAGE_TYPE::PHOTO_SIGNATURE ) {
            int posX, posY;
